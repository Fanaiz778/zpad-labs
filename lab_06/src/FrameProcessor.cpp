...
cv::Mat FrameProcessor::process(
    ...
    drawOverlay(processed, mode, crossX, crossY, brightness);
...
void FrameProcessor::drawOverlay(
    ...
    cv::putText(
        frame,
        "Keys: 1-8 modes | WASD cross | Q/E rotate | +/- zoom | R reset | ESC exit",
...
double FrameProcessor::getMouseWheelZoomFactor() const {
    return mouseWheelZoomFactor;
}
